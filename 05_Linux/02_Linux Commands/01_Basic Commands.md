...
18. file: Give details about the file's format.
...
40. `Editors`:
    ...
    5. nvim: Open in nvim
41. `alias`: We can rename different keywords to be custom commands. This has to be put on config file for fish, bashrc for bash in home directory.
42. `xargs`: Some commands expect arguments for its functioning. xargs lets us pipe the output of another command (stdout) as the arguments of the command.
...
45. ranger: Open ranger - a terminal file system manager
...
49. apropos <words>: Identifies commands related to the words
50. `Networking`:
